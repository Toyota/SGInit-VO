...
def pinhole2Kmat(pinhole: Tuple[float]) -> np.ndarray:
    """ Convert Flatten intrinsics as (fx, fy, cx, cy) to  K matrix with [3,3]"""
    ret = np.eye(3)
    fx, fy, cx, cy = pinhole
    ret[0, 0] = fx
    ret[1, 1] = fy
...
def kmat2pinhole(npz_Kmatrix: np.ndarray) -> np.ndarray:
    """ Convert K matrix with [3,3] to flatten intrinsics as (fx, fy, cx, cy) """
    fx = npz_Kmatrix[0, 0]
    fy = npz_Kmatrix[1, 1]
    cx = npz_Kmatrix[0, 2]
    cy = npz_Kmatrix[1, 2]
